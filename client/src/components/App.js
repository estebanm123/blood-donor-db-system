...
const App = () => {
	...
	const [curUser, setCurUser] = useState({userType: ''}); // set to empty obj //userType: 'Nurse'
	...
	switch (curUser.userType) {
		case ('Nurse'):
			view = <NurseView handleLogout={handleLogout} name={name} id={curUser.id}/>; //set to curUser.id
			break;
		case ('Admin'):
